...
class TestDriver:
    ...
    def setup(
        ...
        self.node_group = utils.get_test_nodegroup(context, labels={}, name="default-worker")
        if auto_scaling_enabled is not None:
    ...
    def test_update_nodegroups_status_delete_complete(self, context, ubuntu_driver, requests_mock):
        ...
        with mock.patch('magnum.objects.NodeGroup.list', return_value=[self.node_group]):
            with requests_mock as rsps:
                rsps.add(
                    self._response_for_cluster_with_machine_deployments(
                        {
