...
pub mod fixtures {
    ...
    pub fn default_values() -> Values {
        Values::builder()
            ...
            .control_plane_flavor_id("ccdb44c7-d184-443c-99ec-e1c7548c3b2f".into())
            .flavor_id("eaaa7b03-f75d-44b1-a475-de8314957fd4".into())
        ...
        for var in &variables {
            match var.name.as_str() {
                ...
                "controlPlaneFlavorID" => {
                    assert_eq!(var.value, json!(default_values().control_plane_flavor_id));
                }
                "flavorID" => {
                    assert_eq!(var.value, json!(default_values().flavor_id));
                }
                "imageRepository" => {
